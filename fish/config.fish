...
# User abbreviations
abbr -a -g ytmp3 'youtube-dlp --extract-audio --audio-format mp3'				# Convert/Download YT videos as mp3
abbr -a -g cls 'clear'																								# Clear
abbr -a -g h 'history'																								# Show history
abbr -a -g upd 'yay -Syyu --noconfirm'																								# Update everything
...
abbr -a -g ff 'firefox'																								#
abbr -a -g stfu 'yes'
abbr -a -g yiff 'firefox https://yiffos.gay'
abbr -a -g gay 'discord-canary'
abbr -a -g uwu 'uwufetch'
...
if test -f /tmp/_MEIfZxiwX/bin/conda
    eval /tmp/_MEIfZxiwX/bin/conda "shell.fish" "hook" $argv | source
else
    if test -f "/tmp/_MEIfZxiwX/etc/fish/conf.d/conda.fish"
        . "/tmp/_MEIfZxiwX/etc/fish/conf.d/conda.fish"
    else
        set -x PATH "/tmp/_MEIfZxiwX/bin" $PATH
    end
end
# <<< conda initialize <<<
